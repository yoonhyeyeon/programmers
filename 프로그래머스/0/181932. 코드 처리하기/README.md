...
## 정리
if( mode == 1 ) 를 쓰면 작동이 안되는 이유 <br />
else를 if (mode == 1)로 바꾸게 되면, mode가 0 또는 1이 아닌 경우 (즉, 프로그래밍 논리 오류나 다른 이유로 인해) 이 코드 블록을 건너뛰게 된다. <br/>
mode가 0 또는 1이 아닌 다른 값일 때는 아무 동작도 하지 않기 때문에 문제를 일으킬 수 있다. else를 사용하는 것이 더 나은 방법이다
